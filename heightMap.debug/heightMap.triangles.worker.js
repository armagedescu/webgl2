...
class HeightMap extends GlVAObjectAsync
{
   ...
   #htmap = null;
   #wireframeMode = false; // Set to false for filled triangles rendering
   ...
   init()
   {
      ...
      // Enable WEBGL_polygon_mode extension for wireframe rendering
      this.polygonModeExt = gl.getExtension('WEBGL_polygon_mode');
      if (this.polygonModeExt) {
         console.log("✅ WEBGL_polygon_mode extension enabled");
      } else {
         console.log("❌ WEBGL_polygon_mode extension not available");
      }
   ...
   drawVao()
   {
      ...
      // Enable wireframe mode using WEBGL_polygon_mode extension
      if (this.polygonModeExt) {
         this.polygonModeExt.polygonModeWEBGL(gl.FRONT_AND_BACK, this.polygonModeExt.LINE_WEBGL);
      }
      ...
      for (let chunk of this.#chunks) {
         ...
         // Draw triangles (will be rendered as wireframe if extension is active)
      ...
      if (this.polygonModeExt) {
         this.polygonModeExt.polygonModeWEBGL(gl.FRONT_AND_BACK, this.polygonModeExt.FILL_WEBGL);
      }
...
let controller; // Initialized in main()
...
async function onMouseEvent(event)
{
   ...
   case "mousemove":
      if (event.shiftKey) {
          // Pan camera position (move both camera and target together)
          let sensitivity = 0.01;
          controller.camera.pos[1] += event.movementY * sensitivity;
          controller.camera.pos[0] -= event.movementX * sensitivity;
          controller.camera.target[1] += event.movementY * sensitivity;
          controller.camera.target[0] -= event.movementX * sensitivity;
      }
      if (event.ctrlKey) {
          // Pan camera target (change what you're looking at)
          let sensitivity = 0.01;
          controller.camera.target[1] -= event.movementY * sensitivity;
          controller.camera.target[0] += event.movementX * sensitivity;
      }
...
async function main()
{
   // Initialize controller with camera settings
   controller = new glcontroller();
   controller.camera.pos[0] = 0;
   controller.camera.pos[1] = 2;
   controller.camera.pos[2] = 5;
   controller.camera.target[0] = 0;
   controller.camera.target[1] = -0.5;
   controller.camera.target[2] = 0;
...
async function heightMapDraw (vao)
{
   ...
   function drawScene(time)
   {
      ...
      if (true)
      {
         ...
         // View matrix: Use controller's camera (which responds to Shift/Ctrl drag)
