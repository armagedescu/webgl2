// VERSION: 3.0 - Onscreen triangles with WEBGL_polygon_mode wireframe
"use strict";
...
{
   elm.addEventListener ( "click",     () => {controller.timer.switchStop();});
...
async function heightMapDraw (vao)
{
   ...
   function drawScene(time)
   {
      ...
      if (true)
      {
         ...
         // Use perspective projection - SET MATRICES FIRST
         vao.model      = modelMatrix;
         vao.view       = viewMatrix;
         vao.projection = projectionMatrix;

         // Update chunk LOD levels based on current matrices
         vao.updateChunkLODs();
