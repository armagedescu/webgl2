...
class GlCanvas
{
   ...
   // add missing attributes if known element type
   ...
   #prepareElement (el)
   {
      ...
      // add attribute data-gl-type="text/glsl-shader" if missing
      if (el.getAttribute ("type") != "text/glsl-shader")
      {
         ...
         if (!type) return null; //not a shader
         el.setAttribute ("data-gl-type", type); //
         return el;
      }
      // not gl shader, generally speaking unreachable code
      return null;
   }
