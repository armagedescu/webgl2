...
class GlBuffer
{
   ...
   attrib2 (size, type, normalized = false, stride = 0, offset = 0)
   {
      ...
      let gl = this.gl;
      this.bindBuffer();
      console.assert (this.location != null);
      gl.vertexAttribPointer     (this.location, size, type, normalized, stride, offset);
      gl.enableVertexAttribArray (this.location);
      return this.location;
   }
...
class GlArrayBufferInfo {
   constructor (gl, name, attribLocation, buffer)
   {
      this.gl                 = gl;
      this.type               = WebGL2S.ARRAY_BUFFER;
      this.buffer             = buffer;
   ...
   withBuffer  (buffer)     {this.buffer     = buffer;     return this;}
   build () {
      let gl = this.gl;
      this.bufferId = gl.createBuffer ();
      this.bindBuffer ();
      gl.bufferData (this.type, this.buffer, this.glDrawType);
      return this.bufferId;
   }
   ...
   bindBuffer  () { this.gl.bindBuffer(this.type, this.bufferId); }
   get attribLocation () {return this.location;}
}
