...
class GlSet {
   ...
   requireColor (dvalue = [ 0.0,  0.0,  0.0,  0.1], dname = "constColor")  { 
       ...
       //console.log (JSON.stringify (this.constColor));
   ...
   get lightDirectionVary  ()    {if (!this.#lightDirection) return null; return this.#lightDirection.vary;} // return this.#_lightDirection;}
   get lightDirection      ()    {return this.#lightDirection;} // return this.#_lightDirection;}
...
class ShaderStrategy {
   ...
   getStrategy () {
      ...
      if (glSet.constColor) {
         ...
         if (glSet.normalsBuffer     && glSet.constColor)           return "0010"; //it has light direction
   ...
   strategy_0010 () {
      ...
      //let lightDirection = glSet.constLightDirection;
      let lightDirection = glSet.selectLightDirection ();
      //lightDirectionName = lightDirection instanceof GlArrayBuffer ? 
      let color = glSet.selectColor ();
      let colorName = (color instanceof GlArrayBuffer) ? color.nameVary : color.name;
      ...
      if (lightDirection.normalize)
         fsBuilder.addMain  (`   float prod = -dot (normalize(${lightDirection.name}),  normalize(${glSet.normalsBuffer.nameVary}) );`);
      else
         fsBuilder.addMain  (`   float prod = -dot (${lightDirection.name},  ${glSet.normalsBuffer.nameVary} );`);
      fsBuilder.addMain  (`   fragColor = vec4 (${colorName}.rgb * prod,  1.0);`);
   }
   strategy_0003 () {
      let [vsBuilder, fsBuilder] = [ this.vsBuilder, this.fsBuilder ];
      let glSet = this.glSet;
...
class GlShapev1
{
   ...
   withTransnation4f (data = null, name = "translate") { //translation can't be normalized
   ...
   lightDirection3 (x, y, z)    { this.gl.uniform3fv (this.glSet.lightDirection.location, ArrayUtil.selectArray(x, y, z)); }
   ...
   #withVertices (data, arrayType, dimension) {
      ...
      glSet.vertexBuffer = GlArrayBuffer.createAndInitBuffer (gl, this.locationCount++, "vertex", data, arrayType, dimension);
      return this;
   }
   ...
   #withNormals (data, arrayType, dimension) {
      ...
      let gl = this.gl, glSet = this.glSet;
      this.bindVertexArray();
      glSet.normalsBuffer = GlArrayBuffer.createAndInitBuffer (gl, this.locationCount++, "normal", data, arrayType, dimension);
   ...
   #withColors (data, arrayType, dimension) {
      ...
      let gl = this.gl, glSet = this.glSet;
      this.bindVertexArray();
      glSet.colorsBuffer = GlArrayBuffer.createAndInitBuffer (gl, this.locationCount++, "color", data, arrayType, dimension);
      return this;
   }
   ...
   buildProgram () {
      ...
      if (this.#programExternal)
       ...
       //update uniformLocations of uniforms always
