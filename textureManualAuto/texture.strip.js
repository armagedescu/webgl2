...
class GlDataRGBTexture2D extends GlTexture
{
   constructor (gl, data)
   {
      super(gl);
   ...
   texImage2D ()
   {
      ...
      const width          = data.width; // pixels
      const height         = data.height; // pixels
      const border         = 0;
      const format         = gl.RGB; //RGB
      const type           = gl.UNSIGNED_BYTE;
      
      this.bindTexture ();
      gl.texImage2D  (this.type, level, internalFormat, width, height, border, format, type, data.RGB);
   }
   bindTexture()
   {
...
function main()
{
   ...
   // Create RGB Texture
   let textureRgb = new GlDataRGBTexture2D (gl, {width:3, height:2, RGB: new Uint8Array([128,  64, 128,      0, 255,   0,    128,  64, 128, 
   ...
   function drawScene(time)
   {
      ...
      textureRgb.data.RGB[4] = (txTime % 1280)  * 0.2; // Change the second pixel value over time
